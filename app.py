...
directory as the script when run from source, or inside the user's data folder
when the app is packaged as an executable.
"""
...
DEFAULT_WORDS_PATH = DATA_DIR / "words.csv"
BUNDLED_CONFIG_PATH = RESOURCE_DIR / "config.json"
BUNDLED_WORDS_PATH = RESOURCE_DIR / "words.csv"
...
def ensure_runtime_files() -> None:
    ...
    bundled_words = BUNDLED_WORDS_PATH.resolve()
    target_words = DEFAULT_WORDS_PATH.resolve()
    if not target_words.exists() and bundled_words.exists() and bundled_words != target_words:
        target_words.write_bytes(bundled_words.read_bytes())

    if not CONFIG_PATH.exists():
        if BUNDLED_CONFIG_PATH.exists() and BUNDLED_CONFIG_PATH.resolve() != CONFIG_PATH.resolve():
            CONFIG_PATH.write_bytes(BUNDLED_CONFIG_PATH.read_bytes())
        else:
...
class JLPTVocabApp(tk.Tk):
    ...
    def get_current_word_file_display(self) -> str:
        ...
        try:
            relative = Path(path).relative_to(DATA_DIR)
            return str(relative)
        except ValueError:
            return str(path)
